...
- 2026-09-06 · [APP.1] · El bucle de tirada es `requestAnimationFrame`, así que una pestaña en segundo plano congela la animación (la partida sigue en `rolling`) y se reanuda al volver · comportamiento aceptado: el timestep fijo con `maxStepsPerFrame` evita el salto de tiempo al recuperar el foco y el resultado ya está decidido por `core`.
- 2026-09-07 · [TEL.1] · `@vercel/analytics` solo se inyecta en producción y fuera de localhost · en `vite preview` el script `/_vercel/insights/script.js` responde 404 y los e2e exigen consola sin errores.
- 2026-09-07 · [MOB.1] · El proyecto `mobile` de Playwright usa el viewport de iPhone 13 sobre **chromium** en vez de WebKit · el CI (y la máquina) solo instalan chromium; lo que se valida es el layout a 390 px, no el motor.
- 2026-09-07 · [SETUP.2] · `makeThrowParams.linvel.x` 9-13 → 5-8 · con el impulso original el 55 % de los dados quedaba en `x > 2.5`, detrás del panel de tienda en escritorio; ahora es el 5 % (mediana `x = −0.7`) y el reposo baja a ≤ 150 pasos (2,5 s).
